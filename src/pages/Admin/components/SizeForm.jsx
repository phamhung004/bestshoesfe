...
const SizeForm = ({ size, onSave, onCancel, isEditing = false }) => {
  ...
  useEffect(() => {
    if (size) {
      setFormData({ sizeName: size.sizeName || '', status: size.status !== undefined ? size.status : true });
    } else {
  ...
  const validate = () => {
    const newErrors = {};
    if (!formData.sizeName.trim()) newErrors.sizeName = 'Tên kích cỡ là bắt buộc';
    setErrors(newErrors);
    ...
    <div className="brand-form-overlay">
      <div className="brand-form-container">
        <div className="brand-form-header">
          <h2 className="form-title">{isEditing ? 'Chỉnh sửa kích cỡ' : 'Thêm kích cỡ mới'}</h2>
        ...
        <form onSubmit={handleSubmit} className="brand-form">
          <div className="form-grid">
            <div className="form-group">
              <label className="form-label">Tên kích cỡ <span className="required">*</span></label>
              <input type="text" name="sizeName" value={formData.sizeName} onChange={handleChange} className={`form-input ${errors.sizeName ? 'error' : ''}`} />
              {errors.sizeName && <span className="error-message">{errors.sizeName}</span>}
            </div>
