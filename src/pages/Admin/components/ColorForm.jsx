...
const ColorForm = ({ color, onSave, onCancel, isEditing = false }) => {
  const [formData, setFormData] = useState({ colorName: '', colorCode: '', status: true });
  const [loading, setLoading] = useState(false);
  const [errors, setErrors] = useState({});

  useEffect(() => {
    if (color) {
      setFormData({ colorName: color.colorName || '', colorCode: color.colorCode || '', status: color.status !== undefined ? color.status : true });
    } else {
    ...
    <div className="brand-form-overlay">
      <div className="brand-form-container">
        <div className="brand-form-header">
          <h2 className="form-title">{isEditing ? 'Chỉnh sửa màu' : 'Thêm màu mới'}</h2>
        ...
        <form onSubmit={handleSubmit} className="brand-form">
          <div className="form-grid">
            <div className="form-group">
              <label className="form-label">Tên màu <span className="required">*</span></label>
              <input type="text" name="colorName" value={formData.colorName} onChange={handleChange} className={`form-input ${errors.colorName ? 'error' : ''}`} />
              {errors.colorName && <span className="error-message">{errors.colorName}</span>}
            </div>

            <div className="form-group">
              <label className="form-label">Mã màu</label>
              <input type="text" name="colorCode" value={formData.colorCode} onChange={handleChange} className="form-input" />
            </div>
