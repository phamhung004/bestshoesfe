...
const BrandList = ({ onEdit, onAdd, refreshTrigger }) => {
  ...
  useEffect(() => {
    ...
    if (searchTerm.trim() === '') {
      ...
      const filtered = brands.filter(brand =>
        ...
        (brand.originCountry && brand.originCountry.toLowerCase().includes(searchTerm.toLowerCase()))
      );
  ...
  const handleToggleStatus = async (brandId, currentStatus) => {
    try {
      ...
      alert('Không thể thay đổi trạng thái thương hiệu. Vui lòng thử lại.');
            ...
            {filteredBrands.length === 0 ? (
              ...
              filteredBrands.map((brand) => (
                <tr key={brand.brandId}>
                  ...
                  <td>
                    <button
                      onClick={() => handleToggleStatus(brand.brandId, brand.status)}
